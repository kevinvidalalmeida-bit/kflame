...
class SpeciesBackend:
    ...
    def eval_mixture_thermal_diffusion(self, T_face, Y_face):
        coefficients = np.empty_like(Y_face)
        for j, temperature in enumerate(T_face):
            self._set_state_unnormalized(temperature, Y_face[:, j])
            coefficients[:, j] = self.gas.thermal_diff_coeffs
        return coefficients
