...
from kflame.reference.plot import generate_clean_plots
...
def _kflame_solve(
    ...
    profile: bool = True,
    verbose: bool = False,
    prev_kflame_data: dict | None = None,
    ...
    from kflame.chemistry.mechanism import load_mechanism  # type: ignore
    from kflame.chemistry.backend import NativeSpeciesBackend  # type: ignore
    ...
    if prev_kflame_data is not None:
        transferred_seed = (
            _resample_kflame_seed(prev_kflame_data, seed_mesh_points)
            if seed_mesh_points is not None
            else prev_kflame_data
        )
...
def _resample_kflame_seed(seed: dict, n_points: int) -> dict:
...
def run(
    ...
    ours = _kflame_solve(
        ...
        prev_kflame_data=prev_kflame_data,
    ...
    summary = {
        "comparison_scope": "direct_cantera_freeflame_vs_kflame_native_freeflame",
        ...
        "case": {
            ...
            "upwind_factor_kflame": float(getattr(case, "upwind_factor", 1.0)),
            "refine": {
                "ratio": case.ratio,
                "slope": case.slope,
                "curve": case.curve,
        ...
        "kflame": {
            "solver_backend": "kflame_native",
            "reference_workflow": KFLAME_REFERENCE_WORKFLOW,
            "external_flamelet_table_tool_used": EXTERNAL_FLAMELET_TABLE_TOOL_USED,
            "ok": ours["ok"],
            "time_s": ours["time_s"],
