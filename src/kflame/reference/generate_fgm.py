"""
generate_fgm_tables_cantera.py  (kflame – Z-C space)
...
Uso:
  python -m kflame reference-fgm --phi-min 0.7 --phi-max 1.4 --n-phi 13 \
         --save-raw-profiles
"""
...
def solve_flame_cantera(
    ...
    if prev_solution is not None:
        ...
        except (ct.CanteraError, ValueError, KeyError) as exc:
            warnings.warn(f"Continuation seed rejected; using a cold guess: {exc}", stacklevel=2)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output-root",    type=str,   default="runs/reference-fgm")
