...
from kflame.flame.state import C_T, C_U, C_Y, build_transient_mask
...
def residual(
    ...
    try:
        ...
        if getattr(backend, 'soret_enabled', False) and not getattr(backend, 'uses_multicomponent_flux', False):
            T_face = .5 * (T[:-1] + T[1:])
            thermal = backend.eval_mixture_thermal_diffusion(T_face, .5 * (Y[:, :-1] + Y[:, 1:]))
            flux -= thermal * ((T[1:] - T[:-1]) / (T_face * np.diff(z)))[None, :]
    except Exception as exc:
        if bool(getattr(problem, "debug_residual_errors", False)):
...
def build_local_jacobian_cache(x: np.ndarray, problem) -> dict:
    ...
    if n_faces and flux_model == 'mixture-averaged' and getattr(backend, 'soret_enabled', False):
        T_face = .5 * (T[:-1] + T[1:])
        soret_face_coeff = backend.eval_mixture_thermal_diffusion(
            T_face, .5 * (Y[:, :-1] + Y[:, 1:])) / T_face[None, :]
    ...
    if n_faces > 0:
        ...
        if cache.get("flux_model") == "multicomponent":
            ...
            if cache['soret_face_coeff'] is not None:
                flux_local -= cache['soret_face_coeff'][:, f0:f1 + 1] * (
                    (T[f0 + 1:f1 + 2] - T[f0:f1 + 1]) / dz)[None, :]
        ...
        for ib in range(n_batch):
            for lf in range(n_faces):
                ...
                for k in range(n_sp):
                    ...
                    if not multicomponent_flux and soret_face_coeff.shape[0] == n_sp:
                        flux_local[ib, k, lf] -= soret_face_coeff[k, f] * (T[ib, jr] - T[ib, jl]) / dz
...
def residual_local_rows_batch(x_batch: np.ndarray, problem, center_j: int,
    ...
    if n_faces > 0:
        ...
        if cache.get("flux_model") == "multicomponent":
            ...
            if cache['soret_face_coeff'] is not None:
                flux_local -= cache['soret_face_coeff'][None, :, f0:f1 + 1] * (
                    (T[:, f0 + 1:f1 + 2] - T[:, f0:f1 + 1]) / dz)[:, None, :]
...
class BlockTridiagJacobian:
    ...
    def matvec(self, vector: np.ndarray) -> np.ndarray:
        ...
        This deliberately stays in block form so a posteriori Jacobian-defect
        checks can evaluate local linear models without a dense matrix.
        """
