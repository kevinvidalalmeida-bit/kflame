...
from kflame.flame.state import build_transient_mask, interpolate_state, pack_state, unpack_state
...
def _factorize_current_linear_system(
    ...
    """Factorize the current Newton or pseudo-transient linear system."""
...
def newton_solve(
    ...
    def full_fun(xv: np.ndarray) -> np.ndarray:
        from kflame.flame.equations import residual as _res
        return _res(
            xv,
            problem,
            rdt=rdt,
...
def solve_free_flame(
    ...
    if x0 is None and bool(opts.multicomponent_bootstrap) and bool(
        getattr(problem.backend, "uses_multicomponent_flux", False) or problem.soret_enabled
    ):
