...
from kava.flame.solver import initial_grid
from kava.flame.state import pack_state, C_Y

from kava.chemistry.mechanism import load_mechanism
from kava.chemistry.thermo import NativeThermo
from kava.chemistry.initialization import fresh_mixture, hp_equilibrium
...
class FreeFlameProblem:
    def __init__(self, case, n_points: int = 8, locs=(0.0, 0.3, 0.5, 1.0), *, mech_data=None):
        ...
        if float(getattr(case, "upwind_factor", 1.0)) != 1.0:
            raise ValueError("The centered/upwind experiment was removed; V2 uses upwind=1.")
