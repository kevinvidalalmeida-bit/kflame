...
Uso:
  python -m kava fgm --phi-min 0.7 --phi-max 1.4 --n-phi 5 \
         --save-raw-profiles
"""
...
from kava.flame.solver import solve_free_flame, SolveOptions
from kava.flame.problem import FreeFlameProblem
from kava.flame.state import interpolate_state, pack_state, unpack_state
from kava.chemistry.mechanism import load_mechanism, resolve_mechanism as _resolve_mechanism
...
def solve_flame_native(
    ...
    def run_once(jacobian_mode: str):
        ...
        if str(args.transport_backend).strip().lower() == "cantera-reference":
            from kava.reference.backend import SpeciesBackend
