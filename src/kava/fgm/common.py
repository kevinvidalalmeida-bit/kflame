...
from kava.chemistry.mechanism import load_mechanism
from kava.chemistry.initialization import NativeMixture
...
def _tabulate_profiles(records: list[Any], n_sp: int, c_grid: np.ndarray) -> dict[str, np.ndarray]:
    """Shared native/reference interpolation; retain field and species arithmetic."""
    n_phi = len(records)
    n_c = int(c_grid.size)
...
def _temperature_dynamic_l2(prediction: np.ndarray, reference: np.ndarray) -> float:
    dynamic_reference = reference - reference[0]
    return float(
        np.linalg.norm(prediction - reference)
        / (np.linalg.norm(dynamic_reference) + np.finfo(float).tiny)
    )
