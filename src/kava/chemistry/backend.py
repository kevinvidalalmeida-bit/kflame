...
from kava.chemistry.mechanism import MechanismData, load_mechanism, R_UNIV
from kava.chemistry.thermo import NativeThermo
from kava.chemistry.transport import NativeTransport
from kava.chemistry.multicomponent import NativeMulticomponentTransport
from kava.chemistry.kinetics import NativeKinetics, _mass_action_product
...
if njit is not None:
    from kava.chemistry.jacobian import grouped_core as _eval_jacobian_grouped_core
...
class NativeSpeciesBackend:
    ...
    def __init__(self, problem, mech_data: MechanismData | None = None):
        ...
        if soret_enabled and not self.uses_multicomponent_flux:
            raise ValueError(
                "La extensión Soret nativa validada de V2 requiere transporte multicomponente; "
                "el cierre Soret mixture-averaged es un modelo distinto aún no implementado."
            )
    ...
    def eval_multicomponent_face_transport(self, T_face: np.ndarray,
        ...
        all face-system assembly and linear algebra below this API are V2's.
        """
