...
interpola suavemente entre ambos estados sobre una malla nominal de ocho nodos
y estima \(u\) a partir de conservación de flujo. Si la temperatura de anclaje
queda entre dos nodos, se inserta un nodo en su posición interpolada para
imponer la condición de fase sin desplazarla a una celda vecina. Una semilla
procedente de continuación sustituye esta conjetura fría y conserva su propia
malla inicial. Este uso puntual de \Cantera para composición y equilibrio no
convierte a \Cantera en el solver de la llama \Vdos; las iteraciones posteriores
usan el backend nativo.
...
En las campañas Soret reportadas se activa un arranque de dos etapas: una llama
nativa promediada por mezcla construye una semilla y un corrector posterior
resuelve el modelo multicomponente con Soret. La primera etapa usa límites de
pendiente y curvatura multiplicados por dos; el corrector recupera los valores
solicitados y ejecuta de nuevo el ciclo de refinamiento. Esta estrategia es una
opción explícita de la configuración Soret, no un paso que se active para el
caso base sin Soret.

El coste reportado incluye ambas etapas y ambas comparten el presupuesto
temporal. Si falla la primera, su perfil no se publica como solución Soret.
Durante las iteraciones del corrector puede reutilizarse el cierre de
transporte de cara entre reconstrucciones del Jacobiano; antes de aceptar, el
residual se reevalúa con transporte multicomponente/Soret actualizado. Una
convergencia sobre la malla provisional no se considera aceptación espacial.
...
guardada. & En la configuración Soret evaluada: llama nativa preliminar
promediada por mezcla y corrector multicomponente.\\
...
Aceptación & Estado finito, norma ponderada, guarda de residual, malla y
dominio. & Mismos controles, tras reevaluar el transporte solicitado sobre la
malla final.\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[
  ...
  \node[common] (cert) at (0,-7.9)
    {Aceptar el solve con propiedades completas actualizadas\\
     Estado finito, control no lineal, malla y dominio};
...
\subsubsection{Rescate con temperatura prescrita}
La jerarquía anterior se ejecuta primero con la ecuación de energía activa.
Si esa etapa completa no obtiene una solución, V2 conserva temporalmente el
perfil térmico del último iterado y resuelve continuidad y especies con
temperatura prescrita. Esta etapa no representa una llama final ni evalúa el
dominio, porque el perfil térmico aún no proviene del balance de energía.
Cuando produce una semilla admisible, se reactiva la energía, se restablece el
anclaje térmico y se repite la resolución completa antes del refinamiento.

Este rescate amplía la cuenca de convergencia sin alterar el problema que se
certifica: solo una solución obtenida después de reactivar la energía puede
avanzar al control espacial. Si la etapa con temperatura prescrita falla o se
agota el presupuesto temporal, la llama se rechaza.
...
La \cref{fig:nonlinear-flow} representa la jerarquía interna de una etapa con
energía activa. Si esa etapa no entrega una solución, se aplica el rescate con
temperatura prescrita descrito antes de reiniciar una etapa completa con energía
activa. Una raíz aceptable en la malla actual todavía debe superar la revisión
de malla y dominio.
...
\begin{tikzpicture}[
  ...
  rescue/.style={proc,draw=naranjaTFM,fill=naranjaTFM!9,text width=5.0cm},
  arr/.style={-{Latex[length=2mm]},thick}]
  ...
  \node[rescue] (fixedT) at (0,-8.1) {Si falla la etapa con energ\'ia activa:\\
    fijar temporalmente \(T\), corregir especies y continuidad,\\
    reactivar energ\'ia y reiniciar la etapa completa};
  ...
  \draw[arr,dashed,naranjaTFM] (be.south)--(fixedT.north);
  \draw[arr,dashed,naranjaTFM] (fixedT.west)--++(-40mm,0)|-(eval.west);
...
\end{tikzpicture}
\caption{Jerarquía interna de una etapa con energía activa. Todo paso aceptado
de Newton, PTC o BE vuelve a la evaluación no lineal; no se refina una llama
solo por haber completado un rescate. El rescate externo con temperatura
prescrita se aplica únicamente si la etapa completa falla.}
...
Cuando están habilitados, el refinador examina \(u\), \(T\) mientras la energía
está activa y cada perfil \(Y_k\). Los perfiles de rango despreciable se omiten
por el umbral interno del refinador, de modo que una traza prácticamente
constante no marca nodos. Los umbrales estrictos usados en la comparación son
...
requieren cambios materiales. La configuración de referencia permite hasta seis
ciclos y un máximo de nodos declarado; si el límite de ciclos se alcanza con
marcas pendientes y se exige convergencia de malla, el solve se rechaza. Si la llama se
...
Una salida del solver no se incorpora automáticamente a la tabla. Es útil
separar el predicado automático de aceptación de las verificaciones físicas
posteriores: el primero decide si la corrida terminó de forma controlada; las
segundas establecen si la solución es suficientemente precisa para un resultado
científico. Cada \(\mathcal A_\ell\) vale verdadero si supera su comprobación y
el operador \(\land\) exige que todas se cumplan. El predicado automático es:
\begin{equation}
  \mathcal A_{\rm solver}=\mathcal A_{\rm estado}\land
  \mathcal A_{\rm NL}\land
  \mathcal A_{\rm malla}\land
  \mathcal A_{\rm dominio}.
  \label{eq:acceptance-predicate}
\end{equation}
La \cref{eq:acceptance-predicate} corresponde a la aceptación final emitida
por el solver, junto con la terminación satisfactoria del ciclo espacial. La
\cref{tab:acceptance-layers} distingue esas condiciones automáticas de los
controles que se calculan al verificar y publicar una campaña.

\begin{table}[htbp]
\centering
\caption{Predicado automático y verificaciones posteriores de una llama individual.}
\label{tab:acceptance-layers}
\begin{tabularx}{\textwidth}{@{}p{0.20\textwidth}YY@{}}
\toprule
\textbf{Capa} & \textbf{Prueba} & \textbf{Error que evita} \\
\midrule
Estado finito & Valores finitos y límites de la \cref{eq:state-bounds}. & NaN, temperatura
...
\midrule
Composición y conservación & \(\sum_kY_k\), flujo másico y balances elementales
reconstruidos. & Pérdida artificial de masa o de elementos. \\
Coherencia física & Perfiles, temperatura quemada, espesor y liberación de calor
comparados sobre una malla común. & Raíz algebraica no representativa de la llama buscada. \\
\bottomrule
\end{tabularx}
\end{table}

La composición se normaliza durante las transferencias de malla y se controla
en las verificaciones de campaña, pero no se utiliza como una cuarta guarda
oculta dentro del predicado de convergencia. Se guardan el estado de aceptación,
norma, residual, número de nodos y tiempo. La aceptación automática no demuestra
por sí sola exactitud frente a un experimento, pero impide confundir ``el proceso
terminó'' con ``la llama es apta para tabular''.
...
certificada. \\
2 & Resolver con la energía activa mediante Newton amortiguado; si no globaliza,
intentar una corrección PTC--SER y, si esta se rechaza, un rescate BE convergido. \\
3 & Si la etapa energética falla sin agotar el tiempo, fijar temporalmente el
perfil térmico, corregir continuidad y especies, reactivar energía y volver al
paso 2. Esta etapa intermedia no decide dominio ni aceptación. \\
4 & Repetir hasta satisfacer norma ponderada, guarda de residual y límites de
estado con la energía activa. \\
5 & Expandir el dominio si un borde no representa un estado asintótico;
transferir el perfil y volver al paso 2. \\
6 & Aplicar ratio, pendiente, curvatura y poda; transferir el estado y volver al
paso 2 si cambia la malla. \\
7 & Emitir una solución solo si convergen el problema no lineal, la malla y el
