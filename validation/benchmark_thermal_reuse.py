...
import numpy as np
import numba
from thermal_reuse_candidate import checked_grouped_core, install
...
def local_check(use_prototype=False):
    results = []
    for mechanism in ('gri30.yaml', 'h2o2.yaml'):
        for pressure in (101325., 1013250.):
            ...
            values = []
            original = backend_module._eval_thermo_kinetics_sparse_numba_core
            try:
                for mode in ('baseline', 'grouped'):
                    method = backend.eval_grid_thermo_kinetics_into
                    if mode == 'grouped':
                        if use_prototype:
                            backend_module._eval_thermo_kinetics_sparse_numba_core = checked_grouped_core
                        else:
                            method = backend.eval_jacobian_thermo_kinetics_into
                ...
                backend_module._eval_thermo_kinetics_sparse_numba_core = original
...
def main():
    ...
    parser.add_argument('--prototype', action='store_true', help='Use the archived isolated prototype instead of the production method')
    ...
    try:
        ...
        pools = None
    output = dict(protocol=__doc__, options=asdict(options(False)), local=local_check(args.prototype), cases={},
                  mode='prototype' if args.prototype else 'production', platform=platform.platform(),
                  ...
                  source_sha256={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest()
                                 for p in (ROOT / 'V2/equations.py', ROOT / 'V2/materiales/species_backend_native.py',
                                           ROOT / 'V2/materiales/thermochemical_jacobian_native.py',
                                           Path(__file__).with_name('thermal_reuse_candidate.py'))})
