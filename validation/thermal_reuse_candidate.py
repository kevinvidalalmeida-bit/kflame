"""Experimental exact T reuse inside one finite-difference node block.

Not imported by production. The caller must supply the node-major [u,T,Y...]
perturbation layout. All but the T column share precisely the same temperature.
Keep density, concentration products, collider/falloff composition dependence
and signed trial concentrations per perturbation. No fastmath or frozen rates.
"""
import math
import numpy as np
...
def checked_grouped_core(*args):
    temperature, y = args[:2]
    nv = y.shape[0] + 2
    assert temperature.size % nv == 0
    blocks = temperature.reshape(-1, nv)
    assert np.all(blocks[:, 2:] == blocks[:, :1])
    return grouped_core(*args)


def install():
    """Scope experimental dispatch to Jacobian precomputation; return restore."""
    import equations
    import species_backend_native as backend
    original = equations._precompute_block_tridiag_center_thermo

    def grouped_precompute(*args, **kwargs):
        core = backend._eval_thermo_kinetics_sparse_numba_core
        backend._eval_thermo_kinetics_sparse_numba_core = checked_grouped_core
        try:
            return original(*args, **kwargs)
        finally:
            backend._eval_thermo_kinetics_sparse_numba_core = core

    equations._precompute_block_tridiag_center_thermo = grouped_precompute

    def restore():
        equations._precompute_block_tridiag_center_thermo = original
