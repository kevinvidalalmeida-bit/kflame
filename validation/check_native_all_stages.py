...
Use with validation/no_cantera on PYTHONPATH. Raw profiles remain in the run
...
import numpy as np
from analyze_soret_validation import elemental_flux_diagnostics
...
def main():
    ...
    sources = sorted((ROOT / 'V2').rglob('*.py')) + sorted((ROOT / 'FGM/scripts').glob('*.py'))
